...
추가 지침:
- 답변이 끝난 뒤,
  "다음으로 도움이 될 수 있는 내용" 제목으로 다음 단계에서 준비하면 좋은 내용이나 이어서 많이 묻는 질문을 최대 3개 bullet point로 정리하세요.
...
def answer_compare(question: str, countries: List[str]) -> str:
    blocks = [build_compare_context(c) for c in countries]
...
추가 지침:
- 문서에 정보가 명시적으로 한 문장에 없더라도, 여러 문단에서 추론 가능한 경우 표에 반영하세요.
- 단, 문서 근거가 전혀 없는 경우만 '검색된 문서 범위 내에서 확인되지 않음'으로 표시하세요.
